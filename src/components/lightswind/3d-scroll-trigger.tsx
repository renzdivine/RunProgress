...
export function ThreeDScrollTriggerContainer({
  ...
  return (
    <ThreeDScrollTriggerContext.Provider value={velocityFactor}>
      <div className={cn("tsst-container", className)} {...props}>
        {children}
      </div>
...
function ThreeDScrollTriggerRowImpl({
  ...
  const BlockContent = useMemo(() => {
    return (
      <div className="tsst-block" style={{ contain: "layout style" }}>
        {childrenArray}
      </div>
    );
  }, [childrenArray]);
  ...
  useEffect(() => {
    ...
    const block = container.querySelector(".tsst-block-measure") as HTMLElement;
    if (block) {
  ...
  return (
    <div
      ref={containerRef}
      className={cn("tsst-scroll", className)}
      {...props}
    >
      <motion.div
        className="tsst-track"
        style={{ transform: xTransform }}
      >
        {Array.from({ length: numCopies }).map((_, i) => (
          <div
            key={i}
            className={i === 0 ? "tsst-block tsst-block-measure" : "tsst-block"}
          >
            {BlockContent}
          </div>
        ))}
